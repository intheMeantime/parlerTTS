...
for (notation, speaker) in description.items() :
    ...
    for index in range(0, len(ds['train'])) :
        prompt = ds['train']['original_text'][index]
        # print(prompt)

        input_ids = tokenizer(speaker, return_tensors="pt").input_ids.to(device)
        try : prompt_input_ids = tokenizer(prompt, return_tensors="pt").input_ids.to(device)
        except :
            pass_prompt.append(index)
            continue
        ...
        for i in range(0, len(audio_arr), chunking):
            ...
            filename = f"E03_{notation}_wiki_{now_index:06}.wav"

            sf.write(f'{path}wiki/{filename}', chunk, sampling_rate)
...
# E03 == parlerTTS
metadata.to_csv(f'{path}wiki/E03_wiki_spoof.csv', sep= ' ', index=False, header=False)
print(f"total duration: {time.time()-start:.2f} sec")
print(f"passed prompts: {pass_prompt}")